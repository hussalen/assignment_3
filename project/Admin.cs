...
namespace assignment_3
{
    public class Admin
    {
        ...
        public void CreateSchedule(DateTime date, TimeSpan startTime, TimeSpan endTime)
        {
            ...
            Console.WriteLine($"Schedule created by Admin {Name}: {date.ToShortDateString()}, {startTime} - {endTime}");
        }

        public void UpdateSchedule(int scheduleId, DateTime date, TimeSpan startTime, TimeSpan endTime)
        {
            if (scheduleId <= 0)
            ...
            Console.WriteLine($"Schedule ID {scheduleId} updated by Admin {Name}: {date.ToShortDateString()}, {startTime} - {endTime}");
        }
        ...
        public Report GenerateReport(string title, string content)
        {
            ...
            if (string.IsNullOrWhiteSpace(content))
                throw new ArgumentException("Report content cannot be null or empty.");

            var report = new Report(title, content);
            Console.WriteLine($"Report generated by Admin {Name}: {report.Title}, ID {report.ReportId}");
            return report;
        }
    }
}
